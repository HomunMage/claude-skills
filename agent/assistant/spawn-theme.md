...
GOAL: ${GOAL}
Load Skill(agent/agentic-hive). Execute the full flow:
1. /agentic-hive plan — initiative the goal into several epics, each epic into stories/tasks
2. /agentic-hive prepare — write config if needed
3. /agentic-hive running — start the hive (queen + bees)
4. Monitor every 2 minutes using ScheduleWakeup(120s)
5. If stuck tasks: split them, restart the hive
...
- Load `Skill(agent/agentic-hive)` — it has the full hive workflow.
- Flow:
